...
type
  ...
  public
    ...
    { Строки исходника, из которых собран абзац. -1 — не текст QV. }
    SrcLine: Integer;
    SrcEnd: Integer;
...
function RenderLaidDocThumb(ADoc: TDocDocument; AWidth, AHeight: Integer;
  ABitmap: TBitmap): Boolean;
...
procedure AddPaintText(Page: TDocLaidPage; X, Y, W, H: Single; const Txt: string;
  const St: TDocRunStyle; ASrcLine: Integer = -1; ASrcCol: Integer = 0;
  ASrcEnd: Integer = -1);
var
  It: TDocPaintItem;
begin
...
procedure SplitTextAtoms(const Txt: string; const St: TDocRunStyle;
  ALine, ABaseCol, AEndLine: Integer; var Atoms: TArray<TAtom>);
var
  I, Start: Integer;
  Piece: string;
  Sz: TSizeF;
      ...
      begin
        ...
        begin
          ...
          AddPaintText(Page, LX, CurY, Line[K].W, Line[K].H, Line[K].Text, Line[K].Style,
            Line[K].SrcLine, Line[K].SrcCol, Line[K].SrcEnd);
...
begin
  ...
  begin
    case Para.Inlines[I].Kind of
      dikText:
        begin
          SplitTextAtoms(Para.Inlines[I].Text, Para.Inlines[I].Style,
            Para.SrcLine, BaseCol, Para.SrcEnd, Atoms);
          BaseCol := BaseCol + Length(Para.Inlines[I].Text);
        end;
      dikTab:
        begin
          A := Default(TAtom);
...
begin
  ...
  if GTagLine >= 0 then
  begin
    Para.SrcLine := GTagLine;
    if GTagEnd >= GTagLine then
      Para.SrcEnd := GTagEnd
    else
      Para.SrcEnd := GTagLine;
  end;
  Blk := TDocBlock.Create;
  Blk.Kind := dbkPara;
  Blk.Para := Para;
...
begin
  Result := False;
  Doc := ADoc;
  if (Doc = nil) or (ABitmap = nil) or (AWidth < 8) or (AHeight < 8) then
    Exit;
